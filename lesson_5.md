...
public Unit() : this.Unit("Unknown Unit")
{
   // Этот конструктор пустой
}
3. Фактическое здоровье (GetRealHealth):  
...
3.3 Внутри метода происходит расчет и выставление в поле здоровья значения: Health -  value * Armor;
